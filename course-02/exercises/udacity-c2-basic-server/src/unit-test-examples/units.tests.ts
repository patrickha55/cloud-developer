...
describe('concat', () => { 
  it('should concat two strings', () => {
    const result = concat('hihi', 'huhu');

    expect(result).to.contain('hihi huhu');
  });

  it('should throw an error if neither string is valid', () => {
    expect(() => { concat('', 'Oh no') }).to.throw('Invalid Parameter');
  });
 })
